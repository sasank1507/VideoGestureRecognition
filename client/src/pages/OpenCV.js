...
const App = () => {
  useEffect(() => {
    ...
    const refreshVideoFeed = () => {
      const videoFeed = document.getElementById('video_feed_open');
      if (videoFeed) {
        videoFeed.src = `http://localhost:8000/video_feed_open?${new Date().getTime()}`;
      }
    };
  ...
  return (
    <div>
      <h1>Hand Gesture Recognition using OpenCV</h1>
      <img id="video_feed" src="http://localhost:8000/video_feed_open" alt="Hand Gesture Feed" />
